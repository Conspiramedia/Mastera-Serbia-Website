...
const i18n = {
    ...
    sr: {
        ...
        photoRemove:         'Ukloni',
        masterFinishTelegram:'✅ Nastavi u Telegramu',
...
// Категория работ приходит с формы на языке страницы, а бот принимает только
// восемь канонических русских названий — specialties в его config.yaml:
//   Мастер на час, Сантехника, Электрика, Ремонт, Мебель, Грузчики, Клининг, Другое
// Всё остальное он отбраковывает, и заявка до мастеров не доходит.
//
// Значения приходят из двух мест:
//   1) <select name="service"> на главных страницах — короткие («Vodoinstalacije»);
//   2) <input type="hidden" name="service"> на лендингах услуг, где категория уже
//      известна. Там она часто с районом: «Мастер на час — Врачар»,
//      «Majstor na sat — Čukarica». Район отрезается в normalizeService().
const BOT_SERVICE_MAP = {
    // ── Мастер на час ──
    'Мастер на час': 'Мастер на час',
    'Majstor na sat': 'Мастер на час', 'Handyman': 'Мастер на час',
    // ── Сантехника ──
    'Сантехника': 'Сантехника',
    'Vodoinstalacije': 'Сантехника', 'Plumbing': 'Сантехника',
    // ── Электрика ──
    'Электрика': 'Электрика',
    'Elektrika': 'Электрика', 'Electrical': 'Электрика',
    // ── Ремонт ──
    'Ремонт': 'Ремонт', 'Бытовой ремонт': 'Ремонт',
    'Popravke': 'Ремонт', 'Kućne popravke': 'Ремонт',
    'Repair': 'Ремонт', 'Home Repairs': 'Ремонт',
    // ── Мебель: сборка и навеска на стену ──
    'Мебель': 'Мебель', 'Сборка мебели': 'Мебель', 'Навеска и монтаж': 'Мебель',
    'Nameštaj': 'Мебель', 'Montaža nameštaja': 'Мебель', 'Kačenje i montaža': 'Мебель',
    'Furniture': 'Мебель', 'Furniture Assembly': 'Мебель', 'Mounting & Installation': 'Мебель',
    // ── Грузчики ──
    'Грузчики': 'Грузчики',
    'Selidbe': 'Грузчики', 'Movers': 'Грузчики',
    // ── Клининг ──
    'Клининг': 'Клининг',
    'Čišćenje': 'Клининг', 'Cleaning': 'Клининг',
    // ── Другое ──
    'Другое': 'Другое', 'Ostalo': 'Другое', 'Other': 'Другое'
};

// Приводит категорию с формы к каноническому названию бота.
// Лендинги районов шлют «Мастер на час — Врачар»: длинное тире с районом
// отрезаем и маппим по базовой части. Если название незнакомо, отдаём
// «Другое» — заявка дойдёт до мастеров, а не потеряется на проверке.
...
// В анкете мастера <select name="specialty"> перечисляет ПРОФЕССИИ на языке
// страницы («Vodoinstalater», «Отделочник», «Plumber»), а бот хранит мастеров по
// тем же восьми каноническим категориям, что и заявки (config.yaml →
// master_registration.specialties). Сравнение идёт функцией specialty_matches
// (specialty == category или startswith), поэтому «Vodoinstalater» не совпадёт
// с категорией «Сантехника» — мастер не увидел бы свои заявки в приоритете
// «по вашему профилю». Переводим профессию в категорию ещё на сайте.
//
// Категории «Мастер на час» в анкете нет: такого пункта в форме не существует,
// он появляется только у заявок клиентов.
const BOT_SPECIALTY_MAP = {
    // ── Сантехника ──
    'Vodoinstalater': 'Сантехника', 'Сантехник': 'Сантехника', 'Plumber': 'Сантехника',
    // ── Электрика ──
    'Električar': 'Электрика', 'Elektricar': 'Электрика',
    'Электрик': 'Электрика', 'Electrician': 'Электрика',
    // ── Ремонт (отделка, «мастер за поправке») ──
    'Majstor za popravke': 'Ремонт', 'Отделочник': 'Ремонт', 'Finisher': 'Ремонт',
    'Moler': 'Ремонт', 'Маляр': 'Ремонт', 'Painter': 'Ремонт',
    // ── Мебель ──
    'Monter nameštaja': 'Мебель', 'Monter namestaja': 'Мебель',
    'Мебельщик': 'Мебель', 'Furniture maker': 'Мебель',
    // ── Клининг ──
    'Čistač': 'Клининг', 'Cistac': 'Клининг',
    'Клининг': 'Клининг', 'Уборщик': 'Клининг', 'Cleaning': 'Клининг', 'Cleaner': 'Клининг',
    // ── Грузчики ──
...
function sendLeadToBot(formData) {
    try {
        ...
        const payload = {
            ...
            category:    normalizeService(get('service')),
...
function sendMasterLeadToBot(formData) {
    try {
        ...
        const payload = {
            ...
            // Каноническая категория (одна из восьми в config.yaml) — для логики бота.
            category:  category,
            // Профессия как её выбрал мастер, плюс опыт — для человека (админа).
